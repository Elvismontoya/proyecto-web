...
router.get('/check-initial', async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ message: 'Error consultando empleados' });
    }
...
router.post('/register-admin', async (req, res) => {
  ...
  try {
    // ¿Ya hay admin activo?
    const { data: empleados, error: errCheck } = await supabase
      .from('empleados')
      .select(`
        id_empleado,
        activo,
    ...
    if (hayAdmin) {
      return res
        .status(403)
        .json({ message: 'Ya existe un administrador registrado.' });
    }

    // Buscar id_rol del admin
    const { data: rolAdmin, error: rolErr } = await supabase
      .from('roles')
      .select('id_rol')
      .eq('nombre_rol', 'admin')
      .single();
    ...
    if (rolErr || !rolAdmin) {
      ...
      return res
        .status(500)
        .json({ message: 'No se encontró el rol admin' });
    }
    ...
    const { data: nuevoEmp, error: insErr } = await supabase
      .from('empleados')
      .insert([
        {
          nombres,
    ...
    if (insErr || !nuevoEmp) {
      ...
      return res
        .status(500)
        .json({ message: 'No se pudo crear el administrador' });
    }
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const { data: emp, error: errEmp } = await supabase
      .from('empleados')
      .select(
        'id_empleado, usuario_login, password_hash, id_rol, activo'
      )

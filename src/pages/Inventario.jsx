...
import { apiGet, apiPut } from "../lib/api"; // 👈 usa el cliente central
...
export default function Inventario() {
  ...
  async function cargarInventario() {
    setCargando(true);
    try {
      const [dataInventario, dataAlertas] = await Promise.all([
        apiGet("/api/inventario"),
  ...
  async function guardarEdicion(id) {
    ...
    try {
      await apiPut(`/api/inventario/${id}`, {
        stock_actual: parseInt(formEdicion.stock_actual, 10),
        stock_minimo: parseInt(formEdicion.stock_minimo, 10),
      });
            ...
            <div className="card-body">
              <div className="d-flex align-items-center">
                <div className="flex-grow-1">
                  <h5 className="text-warning mb-2">⚠️ Alertas de Stock Bajo</h5>
                  <p className="mb-0">
                    {alertas.length} producto(s) tienen stock por debajo del mínimo establecido
                  </p>
                </div>
                <span className="badge bg-warning text-dark fs-6">{alertas.length}</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="card card-soft">
          <div className="card-body">
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              <button className="btn btn-sm btn-outline-brand" onClick={cargarInventario}>
                Actualizar
              </button>
            </div>
                      ...
                      >
                        ...
                        <td className="text-center">
                          {editandoId === item.id_producto ? (
                            ...
                            <span
                              className={
                                item.stock_actual <= 0 ? "text-danger fw-bold" : ""
                              }
                            >
                              {item.stock_actual}
                            </span>
